...
class _CatholicAppState extends State<CatholicApp> {
  ...
  Future<CalendarService> _buildCalendarService() async {
    ...
    final generator = CelebrationGenerator(calendars: [
      GeneralRomanCalendar(),
      SqliteFixedFeastCalendar(rows: feastRows),
    ]);

    return CalendarServiceImpl(engine: LiturgicalEngineImpl(generator: generator));
  }
      ...
      builder: (context, snapshot) {
        if (snapshot.hasError) {
          return MaterialApp(
            ...
            home: Scaffold(
              body: Center(child: Text('Failed to load calendar: ${snapshot.error}')),
            ),
          );
        }
